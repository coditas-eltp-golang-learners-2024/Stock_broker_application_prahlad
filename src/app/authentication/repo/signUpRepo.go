...
func (r *UserRepository) InsertUserInfo(userInfo *models.UserInfo) error {
	query := `
        INSERT INTO userSignUpCredentials (name, email, phoneNumber, panCardNumber, password)
        VALUES ( ?, ?, ?, ?, ?)
    `
	result, err := utils.Db.Exec(query, userInfo.Name, userInfo.Email, userInfo.PhoneNumber, userInfo.PanCardNumber, userInfo.Password)
	if err != nil {
